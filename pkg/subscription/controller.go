...
import (
	...
	subscriptionscheme "github.com/scothis/stream-spike/pkg/client/clientset/versioned/scheme"
...
func NewController(
	...
	subscriptionInformerFactory informers.SharedInformerFactory) *Controller {

	// obtain references to shared index informers for the Deployment and Subscription
	// types.
	deploymentInformer := kubeInformerFactory.Apps().V1().Deployments()
	subscriptionInformer := subscriptionInformerFactory.Samplecontroller().V1alpha1().Subscriptions()
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	if ok := cache.WaitForCacheSync(stopCh, c.deploymentsSynced, c.subscriptionsSynced); !ok {
...
func (c *Controller) syncHandler(key string) error {
	...
	deploymentName := subscription.Spec.DeploymentName
	if deploymentName == "" {
	...
	// Get the deployment with the name specified in Subscription.spec
	deployment, err := c.deploymentsLister.Deployments(subscription.Namespace).Get(deploymentName)
	// If the resource doesn't exist, we'll create it
	if errors.IsNotFound(err) {
		deployment, err = c.kubeclientset.AppsV1().Deployments(subscription.Namespace).Create(newDeployment(subscription))
	}
	...
	// If the Deployment is not controlled by this Subscription resource, we should log
	// a warning to the event recorder and ret
	if !metav1.IsControlledBy(deployment, subscription) {
		msg := fmt.Sprintf(MessageResourceExists, deployment.Name)
		c.recorder.Event(subscription, corev1.EventTypeWarning, ErrResourceExists, msg)
		return fmt.Errorf(msg)
	}

	// If this number of the replicas on the Subscription resource is specified, and the
	// number does not equal the current desired replicas on the Deployment, we
	// should update the Deployment resource.
	if subscription.Spec.Replicas != nil && *subscription.Spec.Replicas != *deployment.Spec.Replicas {
		glog.V(4).Infof("Subscription %s replicas: %d, deployment replicas: %d", name, *subscription.Spec.Replicas, *deployment.Spec.Replicas)
		deployment, err = c.kubeclientset.AppsV1().Deployments(subscription.Namespace).Update(newDeployment(subscription))
	}
	...
	err = c.updateSubscriptionStatus(subscription, deployment)
	if err != nil {
		return err
	}
...
func (c *Controller) updateSubscriptionStatus(subscription *spikev1alpha1.Subscription, deployment *appsv1.Deployment) error {
	// NEVER modify objects from the store. It's a read-only, local cache.
	// You can use DeepCopy() to make a deep copy of original object and modify this copy
	// Or create a copy manually for better performance
	subscriptionCopy := subscription.DeepCopy()
	subscriptionCopy.Status.AvailableReplicas = deployment.Status.AvailableReplicas
	...
	_, err := c.subscriptionclientset.SamplecontrollerV1alpha1().Subscriptions(subscription.Namespace).Update(subscriptionCopy)
	return err
}
...
// newDeployment creates a new Deployment for a Subscription resource. It also sets
// the appropriate OwnerReferences on the resource so handleObject can discover
// the Subscription resource that 'owns' it.
func newDeployment(subscription *spikev1alpha1.Subscription) *appsv1.Deployment {
	...
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      subscription.Spec.DeploymentName,
		...
		Spec: appsv1.DeploymentSpec{
			Replicas: subscription.Spec.Replicas,
