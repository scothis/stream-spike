...
type Monitor struct {
	cache map[streamKey]map[subscriptionKey]spikev1alpha1.SubscriptionSpec
	mutex *sync.Mutex
}

func NewMonitor(spikeInformerFactory informers.SharedInformerFactory) *Monitor {

	subscriptionInformer := spikeInformerFactory.Spike().V1alpha1().Subscriptions()

	monitor := &Monitor{
		cache: make(map[streamKey]map[subscriptionKey]spikev1alpha1.SubscriptionSpec),
		mutex: &sync.Mutex{},
	}
	...
	subscriptionInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			subscription := obj.(spikev1alpha1.Subscription)
			monitor.createOrUpdateSubscription(subscription)
		},
		UpdateFunc: func(old, new interface{}) {
			oldSubscription := old.(spikev1alpha1.Subscription)
			newSubscription := new.(spikev1alpha1.Subscription)
...
func (m *Monitor) Subscriptions(stream string, namespace string) []spikev1alpha1.SubscriptionSpec {
	streamKey := makeStreamKeyWithNames(stream, namespace)
	subscriptionsCache := m.getOrCreateSubscriptionsCache(streamKey)
	m.mutex.Lock()
	subscriptions := make([]spikev1alpha1.SubscriptionSpec, len(subscriptionsCache))
...
func (m *Monitor) getOrCreateSubscriptionsCache(key streamKey) map[subscriptionKey]spikev1alpha1.SubscriptionSpec {
	m.mutex.Lock()
	subscriptionsCache, ok := m.cache[key]
	if !ok {
		subscriptionsCache = make(map[subscriptionKey]spikev1alpha1.SubscriptionSpec)
...
func (m *Monitor) createOrUpdateSubscription(subscription spikev1alpha1.Subscription) {
...
func (m *Monitor) removeSubscription(subscription spikev1alpha1.Subscription) {
	streamKey := makeStreamKey(subscription)
	subscriptionsCache := m.getOrCreateSubscriptionsCache(streamKey)
	subscriptionKey := makeSubscriptionKey(subscription)
	m.mutex.Lock()
...
func makeStreamKey(subscription spikev1alpha1.Subscription) streamKey {
...
func makeSubscriptionKey(subscription spikev1alpha1.Subscription) subscriptionKey {
